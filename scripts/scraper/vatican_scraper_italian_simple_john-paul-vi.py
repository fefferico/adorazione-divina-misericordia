...
BENEDETTO_URL = f"{BASE_URL}/content/benedict-xvi/it"  # SOLO ITALIANO
LIBRARY_PATH = "public/assets/data/benedict-xvi.json"
START_YEAR = 2005
END_YEAR = 2013
    ...
    for folder, display_name, cat_id in CATEGORIES:
        ...
        if has_years:
            for year in range(START_YEAR, END_YEAR + 1):
                url = f"{BENEDETTO_URL}/{folder}/{year}.index.html"
                links = get_links(url)
            ...
            url = f"{BENEDETTO_URL}/{folder}.index.html"
            links = get_links(url)
            for title, link_url in links:
                all_links.append((title, link_url, cat_id))
