...
START_YEAR = 2013
END_YEAR = 2025  # Cambiato da datetime.now().year per evitare anni futuri
...
def scrape_doc(url):
    ...
    try:
        ...
        # Cerca i blocchi vaticanrichtext (specificato come .text.parbase.vaticanrichtext)
        ...
        if len(text_blocks) >= 2:
            ...
            # Selettori possibili per il contenuto (fallback)
            content = (
...
def get_index_links(index_url, category_info):
    ...
    try:
        ...
        for a in soup.select('a[href*=".html"]'):
            ...
            # Validazione: scarta URL truncati o malformati
            if not full_url.startswith('https://www.vatican.va'):
                print(f"    ⚠️  URL malformato (skipped): {full_url[:50]}...")
                continue
            
            if len(full_url) < 30:  # Troppo corto = malformato
                print(f"    ⚠️  URL troppo corto (skipped): {full_url}")
                continue
    ...
    for category_key, category_info in CATEGORIES_MAP.items():
        ...
        if category_info.get("years"):
            ...
            for year in range(START_YEAR, END_YEAR + 1):
                ...
                # Check rapido se l'anno esiste
                quick_check = fetch_with_retry(year_url, max_retries=1, timeout=5)
                if not quick_check:
                    print(f"  ⏭️  {year}: anno non disponibile")
                    continue
                ...
                print(f"  ✓ {year}: {len(links)} link trovati (totale: {count})")
